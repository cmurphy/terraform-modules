...
resource "google_dns_record_set" "A_fulcio" {
  ...
  project = "colleenmurphy-testing-410318" // DELETE BEFORE COMMIT
  provider = google.googleorg
...
resource "google_certificate_manager_dns_authorization" "fulcio_auth" {
  ...
  domain = "fulcio.${var.dns_domain_name}"
}
...
locals {
  hostname = var.dns_domain_name == "" ? "*" : trimsuffix("fulcio.${var.dns_domain_name}", ".")
}
...
resource "google_certificate_manager_certificate" "ssl_certificate" {
  ...
  managed {
    domains = ["fulcio.${var.dns_domain_name}"]
    dns_authorizations = [
...
resource "google_certificate_manager_certificate_map_entry" "fulcio_certificate_map_entry" {
  ...
  hostname     = "fulcio.${var.dns_domain_name}"
}
